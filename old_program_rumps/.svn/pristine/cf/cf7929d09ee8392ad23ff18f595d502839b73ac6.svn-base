...
volatile unsigned int  sleep_count, sleep_time;

volatile unsigned char pressed, sleep, pressed_count, long_pressed;

volatile unsigned char noc_update;

volatile unsigned int speed, prev_speed;
...
//-----------------------------------------------------------------------------------
void EXTERNAL_2_Handler(void) // NOC RX
{
  unsigned int data;
  ...
  if ((NC_NOC_CSR0 & 0x1) == 0x1)
    speed = NC_NOC_RX_BUFF0;
  else if ((NC_NOC_CSR0 & 0x04) == 0x04)
	speed = NC_NOC_RX_BUFF1;
  else if ((NC_NOC_CSR0 & 0x40) == 0x40)
	speed = NC_NOC_RX_BUFF3;

  noc_update = 1;

  if (sleep != 0){
    sleep = 0;
    sleep_count += 1; // to prevent the core back to sleep
  }

  sleep_time = sleep_count + (15*16);
...
void EXTERNAL_4_Handler(void)
{
  ...
  if (((GPIO_IP_SET & 0x1) == 0x1) && (sleep == 0)){
	if (long_pressed == 0)
	  pressed_count += 1;
	sleep_count -= 1; // delay the sleep
  }
  else
	pressed_count = 0;

  if (pressed_count == 64) // approx 4 sec
  {
    pressed_count += 1; // prevent multiple send
	long_pressed = 1;
	IC_NOC_TX_BUFF0 = speed;
	IC_NOC_TX_BUFF1 = speed;
	IC_NOC_TX_BUFF3 = speed;
  }
...
void EXTERNAL_6_Handler(void)
{
  ...
  if ((GPIO_IP_SET & 0x1) == 0x0){ // low
	GPIO_IP_SET = 0x1; // set to high

	if (sleep != 0){
  	  TM_CNT = (0x3D09-0x800)/16;   // set the timer to be almost hit the compare value
  	  sleep_count += 1; // to prevent the core back to sleep
  	}
  }
  else // high
  {
    GPIO_IP_CLR = 0x1; // set to low

    if (sleep == 0 && long_pressed == 0)
      pressed = 1;
    else
      sleep = 0;
