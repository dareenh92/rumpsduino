...
static inline uint32_t libdivide__mullhi_u32(uint32_t x, uint32_t y) { //~RUMPS_MODIFIED
    //uint64_t xl = x, yl = y;
    //uint64_t rl = xl * yl;
    uint64_t rl = mac_umul_32(x, y);
...
static inline int64_t libdivide__mullhi_s64(int64_t x, int64_t y) { //~~ RUMPS modified
#if LIBDIVIDE_VC && LIBDIVIDE_IS_X86_64
    return __mulh(x, y);
#elif HAS_INT128_T
    ...
    uint64_t temp_64b = mac_umul_32(x0, y0);
    const uint32_t x0y0_hi = (uint32_t)( temp_64b >> 32);
    const int64_t t = mac_smul_32(x1, (int32_t)y0) + x0y0_hi;
    const int64_t w1 = mac_smul_32(x0, y1) + (t & mask);
    return mac_smul_32(x1, y1) + (t >> 32) + (w1 >> 32);
    //const uint32_t x0y0_hi = libdivide__mullhi_u32(x0, y0);
...
static uint64_t libdivide_128_div_64_to_64(uint64_t u1, uint64_t u0, uint64_t v, uint64_t *r) { //~~RUMPS_MODIFIED
    // u0 -> rax
    // u1 -> rdx
    // divq
    uint64_t result;
...
static uint64_t libdivide_128_div_64_to_64(uint64_t u1, uint64_t u0, uint64_t v, uint64_t *r) {    
    ...
    rhat = un64 - mac_umul_32(q1, vn1);     // quotient digit, q1.
again1:
    if (q1 >= b || mac_umul_32(q1, vn0) > mac_umul_32(b, rhat) + un1) {
        q1 = q1 - 1;
        rhat = rhat + vn1;
        if (rhat < b) goto again1;
    }

    un21 = mac_umul_32(un64, b) + un1 - mac_umul_32(q1, v);  // Multiply and subtract.

    q0 = un21/vn1;            // Compute the second
    rhat = un21 - mac_umul_32(q0, vn1);     // quotient digit, q0.
again2:
    if (q0 >= b || mac_umul_32(q0, vn0) > mac_umul_32(b, rhat) + un0) {
        q0 = q0 - 1;
        rhat = rhat + vn1;
    ...
    if (r != NULL)                          // If remainder is wanted,
        *r = (mac_umul_32(un21, b) + un0 - mac_umul_32(q0, v)) >> s;    // return it.
    return mac_umul_32(q1, b) + q0;
}
#endif
...
int32_t libdivide_s32_do(int32_t numer, const struct libdivide_s32_t *denom) { //~~ RUMPS modified
    uint8_t more = denom->more;
    ...
    if (more & LIBDIVIDE_S32_SHIFT_PATH) {
        ...
        return q;
    		...
    		int64_t temp_64b = mac_smul_32(denom->magic, numer);
        uint32_t uq = (uint32_t)( temp_64b >> 32);
