...

{
  ...
  while(1){
    ...
    // ## GET_CHNL_DATA
    // Get data from channel - currently from PP (without noise!!)
		while(recv_done==0){
      		
			if( pp_data_ready==1 ){
        for(int i=0; i<4; i++){
          int ppbuff_temp = PP_BUFF;
          recv_sbit[recv_i] = (ppbuff_temp) & 0xff; 
          recv_pbit1[recv_i] = (ppbuff_temp>>8) & 0xff;
          recv_pbit2[recv_i] = (ppbuff_temp>>16) & 0xff;
