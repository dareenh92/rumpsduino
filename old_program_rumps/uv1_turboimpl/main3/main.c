...

{
  ...
  while(1){
    ...
    // ## RECV_NOISEVAR_CALC - receive sbit data to aid in estimating noise variance
    if( flag_pkt_type==IO_NOVAR_HDR ){
      flag_pkt_type = 0; //clear packet type's flag
      
      // send back ACK
      while(noc_NC_txbuff_isfull==1)__NOP(); NC_NOC_TX_BUFF0 = IO_NOVAR_HDR;

      accum temp_calc = 0;

      // aiding noise estimation, E{x^2} part
      for(int i=0; i<256; i++){
        // recv sbit
        ...
        // square
        int64_t temp_64b = mac_smul_32((int32_t)(temp_accumint.int_cont), (int32_t)(temp_accumint.int_cont));
        temp_accumint.int_cont = ( temp_64b >> 15);
        
        // sum
        temp_calc += temp_accumint.accum_cont;
      }

      // send E{x^2} part
      while(noc_NC_txbuff_isfull==1)__NOP();
      temp_accumint.accum_cont = temp_calc;
