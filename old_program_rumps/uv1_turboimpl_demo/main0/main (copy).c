  ...
  while(1){
    ...
    // ((IC_NOC_CSR0 & 0x40)==0x40) && (((IC_NOC_CSR2&0xf0000000)>>28)==0x4)
		if( flag_pkt_type==DSP_LLR_HDR ){
			...
			if( dspsent_i<256 && dspsend_hold==1 && (dec_i&(r_win-1))== 0 && ((dspsent_i>>r_win_mul)-(dec_i>>r_win_mul))<2 )
				dspsend_hold = 0;
        ...
        char uart_buff;
  			...
  			for(short i=0; i<256; i++){
    			...
    			/*temp_accumint.accum_cont = LLR_Le_arr[i];
    ...
    // ## SEND_BITS_DSP
		...
		if(flag_pkt_type==DSP_BITSACK_HDR){
		  flag_pkt_type = 0; //clear packet type's flag
		...
		if( (recv_i>dspsent_i) && (dspsend_hold==0) ){
			...
			//dspsend_hold criteria - dspsent_i is a multiple of r_win
