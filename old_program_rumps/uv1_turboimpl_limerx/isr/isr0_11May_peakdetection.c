...
void EXTERNAL_6_Handler(void)
{
  ...
  static volatile int trig_Ibuff[3] = {0};
  static volatile int trig_Qbuff[3] = {0};
  ...
  if((TM_IF & 0x1) == 0x1){
    ...
    if(rx_state==RX_DETECT){
      if(++stepCount==nextStep){
        ...
        if(++univ_ctr==3){ // detect peak
          univ_ctr = 2;

          if( ( abs_f(trig_Ibuff[1]) > (abs_f(trig_Ibuff[0])+50) ) &&
              ( abs_f(trig_Ibuff[1]) > (abs_f(trig_Ibuff[2])+50) ) ){
            IQtrig = 1;
            tempdump[0] = trig_Ibuff[0];
            tempdump[1] = trig_Ibuff[1];
            tempdump[2] = trig_Ibuff[2];
          }
          else if( ( abs_f(trig_Qbuff[1]) > (abs_f(trig_Qbuff[0])+50) ) &&
                   ( abs_f(trig_Qbuff[1]) > (abs_f(trig_Qbuff[2])+50) ) ){
            IQtrig = 2;
            tempdump[0] = trig_Qbuff[0];
            tempdump[1] = trig_Qbuff[1];
            tempdump[2] = trig_Qbuff[2];
          }

          trig_Ibuff[0] = trig_Ibuff[1];
          trig_Ibuff[1] = trig_Ibuff[2];

          trig_Qbuff[0] = trig_Qbuff[1];
          trig_Qbuff[1] = trig_Qbuff[2];
        } 
        
        if(IQtrig>0){
          trigdump = IQtrig;
          nextStep = (F_SAMPLERATE<<1)-1; // next TIMINGPILOT pair
          univ_ctr = 0;
