  ...
  if((TM_IF & 0x1) == 0x1){
    ...
    if(rx_state==RX_DETECT){
      if(++stepCount==nextStep){
        ...
        if(++univ_ctr==3){ // detect slope trend - without sign
          univ_ctr = 2;
          int diff_i1 = Ibuff[1] - Ibuff[0],
              diff_i2 = Ibuff[2] - Ibuff[1];
          int diff_q1 = Qbuff[1] - Qbuff[0],
              diff_q2 = Qbuff[2] - Qbuff[1];

          if( (abs_f(diff_i1)>100) &&
              ((diff_i1-diff_i2)>=-50) ){
            IQtrig = 1;
            tempdump[0] = Ibuff[0];
            tempdump[1] = Ibuff[1];
        ...
        if(IQtrig>0){
          trigdump = IQtrig;
          nextStep = 1; // next TIMINGPILOT pair
          univ_ctr = 0;
