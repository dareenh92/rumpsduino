...

{
  ...
  while(1){
    if(uart_hd_getchar()=='s'){
      ...
      write_lms6002_reg(0x45, 0x0); // VGA2 gain (0dB)
      ...
      write_lms6002_reg(0x19, 0x26 + 0x80); // tuned vco cap value
  ...
  while(1){
    if(rx_state==RX_SENDUP){
      // debug - dump variable to UART
      uart_hd_putchar(0xf3); __NOP();
      
      uart_hd_putchar(tempdump[0]>>4); __NOP();
      uart_hd_putchar(tempdump[1]>>4); __NOP();
      uart_hd_putchar(tempdump[2]>>4); __NOP();
      uart_hd_putchar(trigdump); __NOP();

      for(int i=0; i<TIMING_PILOTPAIR; i++){
        uart_hd_putchar(ted_dump[i]); __NOP();  
      }

      uart_hd_putchar(0xf3); __NOP();

      for(int i=0; i<768; i++){
        uart_hd_putchar(codeword[i]>>4);
        __NOP(); __NOP();
        codeword[i] = 0; // clean up
      }

      // clean up debug dump variables
      tempdump[0] = tempdump[1] = tempdump[2] = tempdump[3] = 0;
      ted_dump[0] = ted_dump[1] = 0;
      ted_dump[2] = ted_dump[3] = 0;
      trigdump = 0;
