...
accum_int_t codeword[768];
accum_int_t qrecvBuff[768];
  ...
  while(1){
    if(uart_hd_getchar()=='s'){
      ...
      */
      ...
      // ### TEMPORARY - Signal other cores to enter decoding state ###
  ...
  while(1){
    ...
    if(rx_state==RX_PHASECORR){
      ...
      if(tempBuff==RX_PHASECORR){
        ...
        rx_state = RX_SENDUP; // will be RX_DECODE
      }
    }

    // Turbo decoding - gonna be a long one
