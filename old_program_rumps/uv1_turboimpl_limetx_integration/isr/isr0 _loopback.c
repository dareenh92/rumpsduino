...
volatile uint8_t do_rx;
volatile uint8_t rxd_ready;
unsigned char codeword[768];
unsigned char send_pkt[778];
...
void EXTERNAL_6_Handler(void)
{
  ...
  static volatile uint16_t txd_sendctr = 0;
  static volatile uint16_t rxd_ctr = 0;
  //static volatile uint16_t temp_datain;

  if((TM_IF & 0x1) == 0x1){

    // RX Part
    if( do_rx==1 ){
      rxd_buff[rxd_ctr++] = (GPIO_DATAIN>>12) & 0xfff;
    ...
    // TX Part
    if( do_tx==1 ){
      if(send_pkt[txd_sendctr++]==0)
        GPIO_DATAOUT = (GPIO_DATAOUT & 0xfffff000) | 0x800;
      else
        GPIO_DATAOUT = (GPIO_DATAOUT & 0xfffff000) | 0x7ff;

      if(txd_sendctr==1) // start rx after first bit sent
        do_rx = 1;

      if(txd_sendctr==776){ // stop transmit after full pkt sent
        txd_sendctr = do_tx = 0;
