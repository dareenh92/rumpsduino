...
{ 
  ...
  while(1){
    if(uart_hd_getchar()=='s'){
      ...
      // Tx RF
      write_lms6002_reg(0x41, 0x0e); // VGA1 gain (-21dB)
      ...
      */
      ...
      write_lms6002_reg(0x43, 0x90); // Q channel
      // *unit 2
  ...
  /*
  laccum_int_t lacint;
  lacint.laccum_cont = 0.5;
  for(int i=0; i<4; i++){
    uart_hd_putchar(lacint.int_cont >> (i*8));
  ...
  */
  ...
  while(1){ // Main loop
    ...
    // ~~ Temporary ~~
    char ch = uart_hd_getchar();
    char vga1reg = read_lms6002_reg(0x41);
    if(ch==']'){
      vga1reg++;
    ...
    // wait for frame header - specific value
    /*
    while(1){
      if(uart_hd_getchar()==UPPERLAYER_HDR)
        break;
    }
    ...
    unsigned char unpackedFrame[FRAME_NBITS]; // in bytes
    for(int i=0; i<FRAME_NBITS; i++){
      // packed idx i/8, bit i%8
      unpackedFrame[i] = ((oriFrame[i>>3]) >> (i&7));
    }
    */
    // *** END - part3 ***
    ...
    // Add timing pilot - 8 pairs of "1-0"
    ...
    // Add frequency pilot - "1-1"
    sendPkt[TIMINGPILOT_LEN] = 1;
    sendPkt[TIMINGPILOT_LEN+1] = 1;

    // Turbo encoding
    for(int i=0; i<768; i++){
      if(i%3==0)
        sendPkt[i+TIMINGPILOT_LEN+FREQPILOT_LEN] = 0;
      else
        sendPkt[i+TIMINGPILOT_LEN+FREQPILOT_LEN] = 1;
    }
    sendPkt[18] = 1;
    sendPkt[19] = sendPkt[20] = 0;    
    ...
    while(tx_state==TX_ON)__NOP(); // wait till TX finish
