...
/*------------------------------------------------------------------------*/
/* RUMPS401: MMCv3/SDv1/SDv2 (SPI mode) control module                    */
/*------------------------------------------------------------------------*/
/*
/  Copyright (C) 2016
/
...
#define FCLK_SLOW() { SPI_CRSEL = 0x05; }	/* Set SCLK = PCLK / 64 */
#define FCLK_FAST() { SPI_CRSEL = 0x01; }	/* Set SCLK = PCLK / 2 */

/* GPIO0_26:MMC_CS (soft control), GPIO0_30:MMC_SCLK, GPIO0_28:MMC_DO, GPIO0_31:MMC_DI, GPIO0_27:MMC_CD */
#define MMC_CS_PIN 0x04000000
#define MMC_CD_PIN 0x08000000

#define CS_HIGH()	GPIO_BSET = MMC_CS_PIN
#define CS_LOW()	GPIO_BCLR = MMC_CS_PIN

//#define	MMC_CD		((GPIO_DATAIN & MMC_CD_PIN) != MMC_CD_PIN)	/* Card detect (yes:true, no:false, default:true) */
#define	MMC_CD		(0x1)	/* Card detect (SPI mode no card detect) */
...
#define	SPIxENABLE() {\
    ...
    \
    GPIO_OEN_SET = MMC_CS_PIN; /* output */ \
    GPIO_OEN_CLR = MMC_CD_PIN; /* input */ \
    \
	spi_init(MMC_CPHA, MMC_CPOL, MMC_BC8, MMC_IE, MMC_FSB, MMC_SS, MMC_CR); \
    spi_enable(); \
}
...
static void mmc_timer_ms_start (void)
{
	//Timer Settings
	TM_PR = 16000; 		// prescale counter target value (1ms)
	TM_COMR0 = 0xffff;  // no compare
	TM_COMCR = 0x0; 	// continuous count
...
{
	spi_send(dat);    /* put the byte into data register */
						   ...
						   )
{
	...
	spi_init(MMC_CPHA, MMC_CPOL, MMC_BC16, MMC_IE,
			 MMC_FSB, MMC_SS, MMC_CR);	/* Set SPI to 16-bit mode */
	...
	spi_init(MMC_CPHA, MMC_CPOL, MMC_BC8, MMC_IE,
			 MMC_FSB, MMC_SS, MMC_CR);	/* Set SPI to 8-bit mode */
}
