...
static inline int64_t libdivide__mullhi_s64(int64_t x, int64_t y) {//~RUMPS_MODIFIED
#if LIBDIVIDE_VC && LIBDIVIDE_IS_X86_64
    return __mulh(x, y);
#elif HAS_INT128_T
    ...
    const int64_t t = mac_smul_32(x1, (int32_t)y0) + (int64_t)x0y0_hi;
    const int64_t w1 = mac_smul_32((int32_t)x0, y1) + (t & mask);
