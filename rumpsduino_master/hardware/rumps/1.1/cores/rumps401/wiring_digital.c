...
void pinMode( const uint32_t ulPin, uint32_t ulMode )
{
  uint32_t ulPin_new = access_pin(ulPin);
  if(ulMode==OUTPUT){
    GPIO_OEN_SET = 0x1 << ulPin_new;
  }
  else if(ulMode==INPUT){
    GPIO_OEN_CLR = 0x1 << ulPin_new;
    MUXC_PU = 0x0 << ulPin_new;
  }
}
...
void digitalWrite( uint32_t ulPin, uint32_t ulVal )
{
  uint32_t ulPin_new = access_pin(ulPin);
  if(ulVal==LOW){
    GPIO_BSET = 0x1 << ulPin_new;
  }
  else if(ulVal==HIGH){
    GPIO_BCLR = 0x1 << ulPin_new;
  }
}
...
{
  uint32_t ulPin_new = access_pin(ulPin);
  return !((GPIO_DATAIN >> ulPin_new) & 0x1);
}
